 ...
 * la interfaz {@link Runnable}. Dependiendo del número de agente, dispara una
 * serie de transiciones en el monitor de la red de Petri. El proceso de
 * atención
 * se ejecuta en un hilo independiente y continúa hasta que el monitor indique
...
public class AtencionAgente implements Runnable {
   ...
   * de agente (AGENTE1 o AGENTE2), el metodo disparará diferentes transiciones
   * en la red de Petri. El proceso continúa ejecutándose en un ciclo hasta
   * que {@link MonitorInterface#isFinish()} indique que el proceso
   * ha terminado. Si el agente es el AGENTE1 (número 1), se disparan las
   * transiciones T2 y T5. Si el agente es el AGENTE2 (número 2),
   * se disparan las transiciones T3 y T4. En cada ciclo, el
  ...
  public void run() {
    while (true) {
      // Agente Numero 1 o Superor
      if (agente.equals(NumeroDeAgente.AGENTE1)) {
